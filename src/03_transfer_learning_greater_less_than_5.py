...
def update_greater(list_of_labels):
    for idx, label in enumerate(list_of_labels):
        required_condition = label>=5
        list_of_labels[idx] = np.where(required_condition, 1, 0)
...
def main(config_path):
    ...
    y_train_bin,y_test_bin,y_valid_bin = update_greater([y_train,y_test,y_valid])
